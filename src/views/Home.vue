...
<template>
  <div>
    <StracturesFlex :class="configureStore.themes[configureStore.themeNum].text" class=" text-center font-bold max-md:pb-12 bg-transparent" :column="true" items="center">
      <div class="apexFade cursor-default">
        <Icon class="inline max-sm:hidden" width="150" :icon="configureStore.icons.stars"/>
        <Icon class="hidden max-sm:inline" width="100" :icon="configureStore.icons.stars"/>
        <h1 class="transition ease-in duration-500 align-middle inline max-sm:text-4xl sm:text-6xl md:text-6xl lg:text-8xl text-yellow-400" :class="configureStore.themes[configureStore.themeNum].brand">{{$t("home.title")}}</h1>
      </div>
      <StracturesFlex class="apexFade mt-8 cursor-default"  :row="true" justify="center" items="center">
      ...
      <StracturesFlex class="apexFade max-sm:mt-14 mt-20 cursor-default"  :row="true" justify="center" items="center">
        <h2 :class="configureStore.trans" class=" align-middle sm:text-lg md:text-xl lg:text-2xl">{{$t("home.purpose")}} <Icon width="80px" class="inline max-sm:hidden fadeIcons" :icon="configureStore.iconsArr[count]"/><Icon width="50px" class="hidden max-sm:inline fadeIcons" :icon="configureStore.iconsArr[count]"/></h2>
      </StracturesFlex>
    </StracturesFlex>
    <SearchCity class="apexFade" />
    <StracturesFlex v-show="configureStore.open===false" v-if="weather!=null && !weather.hasOwnProperty(`Error`)"  class="font-bold max-sm:mt-2 max-md:mt-5 mt-12 max-md:pb-16" :class="configureStore.themes[configureStore.themeNum].text" :column="true" items="center">
