...
<StracturesFlex :column="true" justify="center" items="center" class="mt-8 max-md:pb-16 space-y-10">
  ...
  <form ref="form" :class="[configureStore.themes[configureStore.themeNum].frame,configureStore.themes[configureStore.themeNum].text,configureStore.trans ]" class=" flex flex-col items-center fadeIn lg:px-20 mt-8" @submit.prevent="sentMail()">
    <label class="block text-2xl max-[300px]:text-base min-[2000px]:text-3xl  max-sm:text-lg p-5" v-for="(item,key) in info" :key="key"  :for="item.id"> {{ (key===0) ? $t('contact.name') : $t('contact.email') }}:
      <input :name="item.label" class="ml-2 outline-3 rounded-2xl outline-none p-1.5 font-normal text-black sm:text-base md:text-lg lg:text-xl max-[350px]:w-[150px] min-[2000px]:text-xl" :type="item.type" :placeholder="item.placeholder"/></label>
    ...
    <textarea name="message" id="issue" class="ml-2 h-[200px] max-md:w-64 max-[350px]:w-56 outline-3 outline-none rounded-2xl capitalize p-1.5 font-normal w-80 text-black sm:text-base md:text-lg lg:text-xl min-[2000px]:text-xl" ></textarea>
