...
<template>
  <div>
    ...
    >
      <Grid v-if="menu===0" sm-columns="3" md-columns="3" lg-columns="4" txl-columns="5" columns="1" class="max-md:text-sm md:text-base justify-items-center pb-2" :class="themes[configureStore.themeNum].headerFooterBgColor">
      ...
      <StracturesFlex v-else  :column="true" :class="themes[configureStore.themeNum].headerFooterBgColor" >
        ...
        <button type="button" @click="menu=0" :class="configureStore.trans" class=" bg-emerald-600 hover:bg-red-700 px-0.5 font-semibold">
