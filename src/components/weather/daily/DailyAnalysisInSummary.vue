...
<template>
    <div class="flex max-md:flex-col l:flex-row md:flex-row lg:flex-row justify-center justify-items-center space-x-8 space-y-4 max-md:divide-y-2 divide-opacity-20 max-sm:mr-3 max-[300px]:mr-6">
      ...
      <StracturesFlex :row="true" justify="center" items="start"  class="max-md:justify-items-center md:justify-items-start content-center items-center fadeIn cursor-default" :class="(windowHeight>2100) ? `text-xl`: null">
        <div>
          <Icon class="inline max-sm:hidden min-[1921px]:hidden " :width="(windowHeight>1500) ? `190` : `150`" :icon="icons[(day===0) ?  weather.currentConditions.icon :  weather.days[day].data.icon]"/>
