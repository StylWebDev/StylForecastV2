...
<template>
  <StracturesFlex :row="true" :wrap="false" items="center" justify="center" class="content-center items-center fadeIn cursor-default">
    <div><Icon class="inline max-sm:hidden min-[1921px]:hidden" width="190" :icon="configureStore.icons[ weather.currentConditions.icon]"/><Icon class="hidden max-sm:inline" width="140" :icon="configureStore.icons[ weather.currentConditions.icon]"/> <Icon class="inline max-[1920px]:hidden" width="250" :icon="configureStore.icons[ weather.currentConditions.icon]"/></div>
    <div>
      ...
      <StracturesFlex :row="true" items="center" class="max-sm:text-sm" :wrap="true">
        ...
        <button class="block my-2 bg-pink-300 px-4 rounded-3xl text-black hover:brightness-150 font-semibold min-[300px]:text-lg max-[300px]:ml-0 max-sm:ml-5 ml-3" :class="configureStore.trans" type="button"><router-link :to="`/weather/${city}/0`">{{$t(`daily.details`)}}</router-link></button>
      </StracturesFlex>
    </div>
