...
<template>
  <div class="grid max-sm:grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 font-semibold mt-[2%] fadeIn">
    <StracturesFlex v-for="(value,index) in weather" :key="index" class=" mt-4 max-[300px]:mx-1 max-md:mx-3 md:mx-4" :class="[(value.data.conditions.length > 20) ? `sm:pr-4` : null , configureStore.themes[configureStore.themeNum].text, configureStore.themes[configureStore.themeNum].frame ]" :column="true" items="center" justify="around">
      ...
      <button v-if="index<4" class="block my-2 bg-pink-300 px-4 rounded-3xl text-neutral-900 hover:brightness-150" :class="configureStore.trans" type="button"><router-link :to="`/weather/${city}/${index+1}`">{{$t(`daily.details`)}}</router-link></button>
    </StracturesFlex>
  </div>
</template>
