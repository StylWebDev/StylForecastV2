...
<template>
  <div class="grid max-sm:grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 font-semibold mt-[2%] fadeIn">
    <StracturesFlex v-for="(value,index) in weather" :key="index" class=" mt-4 max-[300px]:mx-1 max-md:mx-3 md:mx-4" :class="[(value.data.conditions.length > 20) ? `sm:pr-4` : null , themes[configureStore.themeNum].text, themes[configureStore.themeNum].frame ]" :column="true" items="center" justify="around">
      ...
      <StracturesFlex v-if="index<4"  :row="true" items="center" justify="center" class="mt-6 content-center items-center cursor-default">
        <div><Icon class="inline max-sm:hidden min-[1921px]:hidden" width="160" :icon="configureStore.icons[value.data.icon]"/><Icon class="hidden max-sm:inline" width="120" :icon="configureStore.icons[value.data.icon]"/> <Icon class="inline max-[1920px]:hidden" width="200" :icon="icons[value.data.icon]"/></div>
        <div>
      ...
      <button v-if="index<4" class="block my-2 px-4 rounded-3xl hover:brightness-150" :class="[themes[configureStore.themeNum].detailsBtn , trans]" type="button"><router-link :to="`/weather/${city}/${index+1}`">{{$t(`daily.details`)}}</router-link></button>
    </StracturesFlex>
  </div>
</template>
